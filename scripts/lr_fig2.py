...
fig, axs = plt.subplots(1,len(dirt), figsize=(6*cm * len(dirt), 6*cm), squeeze=False)

plot_groups = quality_counts.groupby(['Dataset'], sort=False)
for k,(title, quality_counts) in enumerate(plot_groups):
    ...
    axs[0][k].set_title(title[0], fontsize=fs)
    axs[1][k].sharex(axs[0][k])
    axs[1][k].set_xlabel("> 90%, 70%, 50% complete\n and <5 % contamination")
    axs[0][k].set_xlabel("")
#    if k > 0:
